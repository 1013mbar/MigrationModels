...
def avgMove(pop1,pop2,distance):
    coeff = 9.2
    return((pop1*pop2)/np.exp(distance*coeff))
...
if __name__ == '__main__':
    ...
    for grid in grids:
        ...
        plt.savefig("gravExp/gravExp_"+ str(k) +  "_randDecreasingUrban_coeff"+str(coeff)+ "_avgMig" + str(np.round(np.mean(migrants))) + "_pop"+ str(np.round(np.sum(np.absolute(gridT7))))+ ".png",dpi=fig.dpi)
        #plt.show()
        k += 1
